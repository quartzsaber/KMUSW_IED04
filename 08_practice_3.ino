...
// Use last measured distance to filter out abnormal values
  float dist = (dist_raw != 0.0) ? dist_raw : dist_last;
  dist_last = dist;
...
// control LED based on the distance
  int valuei = abs(dist - 200) * (255.0 / 100.0);
  byte value = max(0, min(255, valuei));
